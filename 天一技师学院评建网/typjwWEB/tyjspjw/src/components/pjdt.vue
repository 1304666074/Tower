      ...
      <div class="page_main">
        ...
        <div class="page_content">
          <ul class="dateListBox">
            <li v-for="i in pjdtList" :key="i.ID"><router-link :to="{path:'/Detail',query:{ID:i.ID}}">
...
export default {
  ...
  methods:{  
    ...
    pageMove: function(p){
      ...
      this.curPage = p;
      this.pjdtList = Base.GetNewsInfoByCid(44,this.curPage,this.pageSize);
    }
  },
  created() {
    // 子页面banner图
    this.pageBanner = "background:url(" + Base.GetFilesInfoByCid(49)[0].FileUrl+") center no-repeat;";
    this.pageHeader = Base.GetColumnByid(44);
    this.pjdtList = Base.GetNewsInfoByCid(44,this.curPage,this.pageSize);
    this.pageCount = Math.ceil(this.pjdtList[0].Total/this.pageSize);
  }
}
</script>
