    ...
    <div class="page_box">
      ...
      <div class="page_main">
        ...
        <div class="page_content">
          <ul class="listBox">
            <li v-for="(i,index) in tzggList" :key="index" ><router-link :to="{path:'/Detail',query:{ID:i.ID}}">
              <span>{{ i.Title }}</span>
...
export default {
  ...
  methods: {
    ...
    pageMove: function(p) {
      ...
      this.curPage = p;
      this.tzggList = Base.GetNewsInfoByCid(43, this.curPage, this.pageSize);
    }
  },
  created() {
    // 子页面banner图
    this.pageBanner =
      "background:url(" +
      Base.GetFilesInfoByCid(49)[0].FileUrl +
      ") center no-repeat;";
    this.pageHeader = Base.GetColumnByid(43);
    this.tzggList = Base.GetNewsInfoByCid(43, this.curPage, this.pageSize);
    this.pageCount = Math.ceil(this.tzggList[0].Total / this.pageSize);
  }
};
</script>
