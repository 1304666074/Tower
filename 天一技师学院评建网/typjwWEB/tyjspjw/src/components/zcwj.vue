...
export default {
  ...
  methods:{  
    ...
    pageMove: function(p){
      ...
      this.curPage = p;
      this.zcwjList = Base.GetFilesInfoByCid(46,this.curPage,this.pageSize);
    }
  },
  created() {
    // 子页面banner图
    this.pageBanner = "background:url(" + Base.GetFilesInfoByCid(49)[0].FileUrl+") center no-repeat;";
    this.pageHeader = Base.GetColumnByid(46);
    this.zcwjList = Base.GetFilesInfoByCid(46,this.curPage,this.pageSize);
    this.pageCount = Math.ceil(this.zcwjList[0].Total/this.pageSize);
  }
}
</script>
